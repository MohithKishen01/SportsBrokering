...
public class SBOBetWinSettledSM extends SBOBetWinSettled
{
    SBOListWinSuper m_oSuperWindow = null;
	
	public SBOBetWinSettledSM (Document oDocument, HCLoginData oLoginData, CloseableHttpClient oHttpClient, String oWindowKey)
    {
	...
	protected void fetchBetsFromWebsite ()
	{
		if (isReadyState () && getSuperWindow () != null)
		{
			...
			while (oSuperWindow.moveNextMaster ())
			{
				while (oSuperWindow.moveNextAgent ())
				{
					while (oSuperWindow.moveNextMember ())
					{
						...
						SBOBetParserSettled oParser = new SBOBetParserSettled (getLogFilePath (), getWindowKey (), hashParamsEx);
			...
			{
				log ("SBOBetWinSettledSM:fetchBetsFromWebsite:Exception:" + oException.toString ());
			}
		}
	}
	...
	protected SBOListWinSuper getSuperWindow ()
	{
		if (m_oSuperWindow == null)
		{
			m_oSuperWindow = new SBOListWinSuper (getDocument (), getLoginData (), getHttpClient (), SBOConstants.kWK_SettledBet);
